...
if selected_data == '--':
    ...
    df_rules = DataFrame()
    if uploaded_file:
        df_rules = make_data_full(uploaded_file)
...
#**********************************************************************************************************
    ...
    def make_data_3keta(file):
        ...
        freq_items1 = apriori(association_df, min_support=0.0003, use_colnames=True) 
        # min_support 閾値 その組み合わせの全体の構成比
        freq_items1.sort_values('support', ascending=False) 
    ...
    # ***ファイルアップロード 今期***
    uploaded_file = st.sidebar.file_uploader('今期', type='xlsx', key='3keta')
    df_rules = DataFrame()
    if uploaded_file:
        df_rules = make_data_3keta(uploaded_file)
